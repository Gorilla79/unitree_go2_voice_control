...
static inline void pre_balance(SportClient& c){ c.BalanceStand(); msleep(600); } // BalanceStand는 잠금 해제 균형서기 :contentReference[oaicite:4]{index=4}
...
int main(int argc, char** argv){
  ...
  unitree::robot::ChannelFactory::Instance()->Init(0, ifname);  // 예제와 동일 패턴 :contentReference[oaicite:5]{index=5}

  SportClient cli;
  cli.SetTimeout(10.0f);
  cli.Init(); // 반환값 없음(문서 예제 동일) :contentReference[oaicite:6]{index=6}
  ...
  while(!stop_flag){
    ...
    while(iss >> id && !stop_flag){
      ...
      switch(id){
        case 1:  ret = cli.StandUp(); break;           // int32_t 반환 :contentReference[oaicite:7]{index=7}
        case 2:  ret = cli.StandDown();                // int32_t 반환(웅크림) :contentReference[oaicite:8]{index=8}
                 if (ret==0) { pending_standup = true; std::cout << "[RULE] StandUp pending (특수 신호 대기)\n"; }
                 break;
        case 3:  ret = cli.Sit();                      // 특수동작: 앉기(예제 참조) :contentReference[oaicite:9]{index=9}
                 if (ret==0) { pending_risesit = true; std::cout << "[RULE] RiseSit pending (특수 신호 대기)\n"; }
                 break;
        case 4:  ret = cli.RiseSit(); break;
        case 5:  ret = cli.BalanceStand(); break;      // 균형서기(잠금해제) :contentReference[oaicite:10]{index=10}
        case 6:  ret = cli.RecoveryStand(); break;
        case 7:  ret = cli.StopMove(); break;

        case 8:  ret = cli.Hello(); break;             // 인사/스트레칭/하트/행복/절 등 특수동작군은 문서에 명시 :contentReference[oaicite:11]{index=11}
        case 9:  ret = cli.Stretch(); break;
        case 10: ret = cli.Content(); break;
        case 11: ret = cli.Heart(); break;
        case 12: ret = cli.Scrape(); break;

        case 13: pre_balance(cli); ret = cli.FrontJump(); break;
